...
class AssistantUIV2 {
    ...
    async fallbackToTraditionalSearch(query) {
        console.log('🔄 الانتقال إلى البحث التقليدي...');
        
        try {
            let response;
            
            if (query.includes('مصنع')) {
                response = {
                    text: `🏭 **مصنع إنتاج الهيدروجين الأخضر**\n\n${'═'.repeat(60)}\n\n` +
                          `📋 **التراخيص المطلوبة:**\n` +
                          `1. ترخيص من وزارة الصناعة والتجارة\n` +
                          `2. موافقة من الهيئة العامة للاستثمار\n` +
                          `3. تصريح من وزارة البيئة\n` +
                          `4. ترخيص من الشركة المصرية لنقل الكهرباء\n\n` +
                          `✅ **القرار 104:**\n` +
                          `نعم، مشمول في القطاع (أ) - حافز 50% من التكلفة\n\n` +
                          `📍 **المناطق المقترحة:**\n` +
                ...
                response = {
                    text: `🤔 **لقد حدث خطأ في النظام المتقدم**\n\n` +
                          `✅ جاري استخدام النظام التقليدي...\n\n` +
                          `💡 **استفسارات مقترحة:**\n` +
                          `• "تراخيص فندق 5 نجوم"\n` +
                          `• "منطقة العاشر من رمضان"\n` +
                          `• "ما هو القرار 104"\n\n` +
                          `🔧 **جاري إصلاح النظام الذكي...**`,
                    type: 'error',
                    confidence: 0.6
                };
            }
            ...
            this.showError('تعذر معالجة الاستعلام حتى بالطريقة التقليدية.');
        }
    }
